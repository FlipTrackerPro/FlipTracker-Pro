...
function buildInventorySprint3_() {
  ...
  // Calculated columns remain live even when Inventory is edited directly.
  const pp=columnLetter3_(c['Purchase Price']), qty=columnLetter3_(c['Quantity']);
  const tax=columnLetter3_(c['Tax Paid']), ship=columnLetter3_(c['Acquisition Shipping']);
  const totalCol=columnLetter3_(c['Total Cost']), listedCol=columnLetter3_(c['Listed Price']);
  const expectedCol=columnLetter3_(c['Expected Sale Price']), purchaseDateCol=columnLetter3_(c['Purchase Date']);
  const descCol=columnLetter3_(c['Description']), statusCol=columnLetter3_(c['Status']);
  s.getRange(2,c['Total Cost'],FTP3.ROWS,1).setFormulaR1C1(
    '=IF(RC'+c['Description']+'="","",N(RC'+c['Purchase Price']+')*MAX(1,N(RC'+c['Quantity']+'))+N(RC'+c['Tax Paid']+')+N(RC'+c['Acquisition Shipping']+'))');
  s.getRange(2,c['Days in Inventory'],FTP3.ROWS,1).setFormulaR1C1(
    '=IF(RC'+c['Purchase Date']+'="","",MAX(0,TODAY()-RC'+c['Purchase Date']+'))');
  s.getRange(2,c['Projected Profit'],FTP3.ROWS,1).setFormulaR1C1(
    '=IF(RC'+c['Description']+'="","",IF(N(RC'+c['Listed Price']+')>0,N(RC'+c['Listed Price']+'),N(RC'+c['Expected Sale Price']+'))-N(RC'+c['Total Cost']+'))');
  s.getRange(2,c['Projected ROI %'],FTP3.ROWS,1).setFormulaR1C1(
    '=IFERROR(RC'+c['Projected Profit']+'/RC'+c['Total Cost']+',"")');
...
function saveInventoryItem(form) {
  ...
  s.getRange(row,c['Total Cost']).setFormulaR1C1('=N(RC'+c['Purchase Price']+')*MAX(1,N(RC'+c['Quantity']+'))+N(RC'+c['Tax Paid']+')+N(RC'+c['Acquisition Shipping']+')');
  s.getRange(row,c['Days in Inventory']).setFormulaR1C1('=MAX(0,TODAY()-RC'+c['Purchase Date']+')');
  s.getRange(row,c['Projected Profit']).setFormulaR1C1('=IF(N(RC'+c['Listed Price']+')>0,N(RC'+c['Listed Price']+'),N(RC'+c['Expected Sale Price']+'))-N(RC'+c['Total Cost']+')');
  s.getRange(row,c['Projected ROI %']).setFormulaR1C1('=IFERROR(RC'+c['Projected Profit']+'/RC'+c['Total Cost']+',"")');
...
function onOpen() {
  SpreadsheetApp.getUi().createMenu('FlipTracker Pro')
    .addItem('Install / Repair v0.5.2','installFlipTrackerPro')
...
function installFlipTrackerPro() {
  ...
  p.setProperty('FTP_SCHEMA_VERSION','5.2'); p.setProperty('FTP_APP_VERSION',FTP3.VERSION);
  goToDashboardSprint3();
  SpreadsheetApp.getActive().toast('FlipTracker Pro v0.5.2 is ready.','FlipTracker Pro',6);
}
