const FTP3 = {
  ...
  VERSION: '0.4.4 Automatic Sale Workflow',
  ROWS: 1000,
  SHEETS: {
...
function cancelPendingSale3_(itemId){
  ...
  if(item && String(item.values[18])==='Sale Pending'){
    ...
    sheet3_(FTP3.SHEETS.INVENTORY).getRange(item.row,19).setValue(previous);
...
function installFlipTrackerPro() {
  ...
  p.setProperty('FTP_SCHEMA_VERSION','4.4'); p.setProperty('FTP_APP_VERSION',FTP3.VERSION);
