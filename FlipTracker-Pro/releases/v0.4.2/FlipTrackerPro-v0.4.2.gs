...
function showRecordSaleForm() {const items=activeInventoryChoices3_();if(!items.length){SpreadsheetApp.getUi().alert('There are no active inventory items available to sell.');return;}const html=HtmlService.createHtmlOutput(saleFormHtml3_(items,packagingChoices3_())).setWidth(680).setHeight(820);SpreadsheetApp.getUi().showModalDialog(html,'Record Sale');}
function saleFormHtml3_(items,packages) {
  const itemOptions=items.map(x=>`<option value="${x.id}">${x.label}</option>`).join('');
...
function saveSale3_(form) {
  ...
  try{
    const item=inventoryItemById3_(form.itemId);if(!item)throw new Error('The selected inventory item was not found.');const inv=item.values;
    ...
    const values=[nextId3_(FTP3.SHEETS.SALES,1,'SAL'),form.itemId,saleDate,form.marketplace||'',salePrice,shippingCharged,shippingActual,pkg.total,marketFees,paymentFees,promotion,taxCollected,itemCost,grossRevenue,sellingCosts,netProceeds,realizedProfit,roi,days,form.buyer||'',form.trackingNumber||'',form.notes||'',new Date(),form.boxId||'',num3_(form.boxQty),form.bubbleId||'',num3_(form.bubbleQty),form.mailerId||'',num3_(form.mailerQty),form.tapeId||'',num3_(form.tapeQty),form.otherPackagingId||'',num3_(form.otherPackagingQty),'Yes'];
    ...
    const inventory=sheet3_(FTP3.SHEETS.INVENTORY);inventory.getRange(item.row,19).setValue('Sold');inventory.getRange(item.row,27).setValue(new Date());refreshDashboardSprint3();
...
function onOpen() {
  SpreadsheetApp.getUi().createMenu('FlipTracker Pro')
    .addItem('Install / Repair v0.4.2','installFlipTrackerPro')
