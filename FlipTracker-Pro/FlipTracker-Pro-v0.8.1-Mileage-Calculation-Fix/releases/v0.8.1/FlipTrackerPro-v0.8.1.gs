...
const FTP3 = {
  ...
  MILEAGE_HEADERS: [
    ...
    'Round Trip','Vehicle','Start Odometer','End Odometer',
...
function applyFlipTrackerNumberFormats3_() {
  ...
  const specs=[
    ...
    },
    {
      ...
      integer:['Start Odometer','End Odometer','Total Kilometres','Business Kilometres'],
      percent:[]
    },
    {
...
function repairWholeNumberValues3_() {
  const specs=[
    ...
    [FTP3.SHEETS.MILEAGE,['Start Odometer','End Odometer','Total Kilometres','Business Kilometres']],
...
function onEdit(e) {
  ...
  const sheet=e.range.getSheet();
  if(sheet.getName()===FTP3.SHEETS.MILEAGE && e.range.getRow()>1){
    const map=headerMap3_(sheet);
    if([map['Start Odometer'],map['End Odometer']].indexOf(e.range.getColumn())>=0)setMileageTotalFormula81_(sheet,e.range.getRow());
    return;
  }
