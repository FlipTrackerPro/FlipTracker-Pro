...
function setExpenseCalculationFormulas3_(sheet,row,map) {
  ...
  sheet.getRange(row,map['Deductible Amount'])
    .setFormula('=IF('+total+row+'="","",ROUND('+total+row+'*IF('+business+row+'="",1,'+business+row+'),2))')
    .setNumberFormat('$#,##0.00;[Red]-$#,##0.00');
}
...
function repairExpenseCalculations3_() {
  ...
  for(let row=2;row<=rows+1;row++){
    ...
    deductibleFormulas.push(['=IF('+total+row+'="","",ROUND('+total+row+'*IF('+business+row+'="",1,'+business+row+'),2))']);
  }
...
function installFlipTrackerPro() {
  ...
  repairWholeNumberValues3_();
  applyFlipTrackerNumberFormats3_();
  p.setProperty('FTP_SCHEMA_VERSION','5.8'); p.setProperty('FTP_APP_VERSION',FTP3.VERSION);
  goToDashboardSprint3();
  SpreadsheetApp.getActive().toast('FlipTracker Pro v0.5.8 is ready.','FlipTracker Pro',6);
}
...
function upgradeFlipTrackerPro() {
  ...
  applyFlipTrackerNumberFormats3_();
  p.setProperty('FTP_SCHEMA_VERSION','5.8'); p.setProperty('FTP_APP_VERSION',FTP3.VERSION);
  SpreadsheetApp.getActive().toast('FlipTracker Pro upgraded to schema 5.8.','FlipTracker Pro',6);
}
