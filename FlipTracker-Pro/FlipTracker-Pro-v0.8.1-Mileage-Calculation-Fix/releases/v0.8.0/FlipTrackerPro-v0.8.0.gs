...
const FTP3 = {
  ...
  SHEETS: {
    ...
    ADMIN:'Admin', SETTINGS:'Settings', TAX_CENTRE:'Tax Centre', ACCOUNTANT_EXPORT:'Accountant Export', PACKAGING_LISTS:'Packaging Lists', MILEAGE_LISTS:'Mileage Lists', SHIPPING_SETTINGS:'Shipping Settings'
  },
  COLORS: {
  ...
  SALES_HEADERS: [
    ...
    'Shipping Carrier','Shipping Service','Suggested Packaging','Estimated Shipping','Shipping Variance'
  ],
  EXPENSE_HEADERS: [
...
function buildSalesSprint3_() {
  ...
  ['Package Length (cm)','Package Width (cm)','Package Height (cm)'].forEach(name=>s.getRange(2,c[name],FTP3.ROWS,1).setNumberFormat('0.0'));
  s.getRange(2,c['Package Weight (kg)'],FTP3.ROWS,1).setNumberFormat('0.00');
  ['Estimated Shipping','Shipping Variance'].forEach(name=>s.getRange(2,c[name],FTP3.ROWS,1).setNumberFormat('$#,##0.00;[Red]-$#,##0.00'));
...
function buildMileageSprint3_() {
  ...
  ['Start Odometer','End Odometer','Google Maps Distance','Total Kilometres','Business Kilometres'].forEach(h=>s.getRange(2,map[h],FTP3.ROWS,1).setNumberFormat('0'));
...
function showRecordMileageForm() {
  ...
  <div><label>Google Maps distance (km)</label><input type="number" step="1" min="0" name="mapsDistance"><div class="hint">Optional; round trip doubles it.</div></div><div><label>Business kilometres</label><input type="number" step="1" min="0" name="businessKm"><div class="hint">Blank uses full trip distance.</div></div></div>
...
function saveMileage3_(form) {
  ...
  const odometerDistance=Math.max(0,odoEnd-odoStart), mapsEntered=form.mapsDistance!==''&&form.mapsDistance!=null;
...
function buildDashboardSprint3_() {
  ...
  s.getRange('A1:H2').merge().setValue('FlipTracker Pro — v0.7 Smart Mileage').setBackground(FTP3.COLORS.NAVY).setFontColor(FTP3.COLORS.WHITE).setFontSize(22).setFontWeight('bold').setHorizontalAlignment('center').setVerticalAlignment('middle');
  ...
  const mClaim=sheetColumnLetter3_(FTP3.SHEETS.MILEAGE,'Claim Amount'),mDate=sheetColumnLetter3_(FTP3.SHEETS.MILEAGE,'Date'),mPurpose=sheetColumnLetter3_(FTP3.SHEETS.MILEAGE,'Business Purpose'),mKm=sheetColumnLetter3_(FTP3.SHEETS.MILEAGE,'Business Kilometres'),mEnd=sheetColumnLetter3_(FTP3.SHEETS.MILEAGE,'End Location'),mVehicle=sheetColumnLetter3_(FTP3.SHEETS.MILEAGE,'Vehicle');
  const cards=[
  ...
  s.getRange('A45:D45').merge().setValue('Business Kilometres by Destination');header3_(s.getRange('A45:D45'));
  s.getRange('A46').setFormula(`=QUERY({Mileage!${mEnd}2:${mEnd},Mileage!${mKm}2:${mKm}},"select Col1,sum(Col2) where Col1 is not null group by Col1 order by sum(Col2) desc label Col1 'Destination',sum(Col2) 'Business KM'",0)`);
  s.getRange('E45:H45').merge().setValue('Business Kilometres by Vehicle');header3_(s.getRange('E45:H45'));
  s.getRange('E46').setFormula(`=QUERY({Mileage!${mVehicle}2:${mVehicle},Mileage!${mKm}2:${mKm}},"select Col1,sum(Col2) where Col1 is not null group by Col1 order by sum(Col2) desc label Col1 'Vehicle',sum(Col2) 'Business KM'",0)`);
...
function installFlipTrackerPro() {
  ...
  repairWholeNumberValues3_();
  applyFlipTrackerNumberFormats3_();
  p.setProperty('FTP_SCHEMA_VERSION','8.0'); p.setProperty('FTP_APP_VERSION',FTP3.VERSION);
  goToDashboardSprint3();
  SpreadsheetApp.getActive().toast('FlipTracker Pro v0.8.0 Smart Shipping is ready.','FlipTracker Pro',6);
}
...
function upgradeFlipTrackerPro() {
  ...
  applyFlipTrackerNumberFormats3_();
  p.setProperty('FTP_SCHEMA_VERSION','8.0'); p.setProperty('FTP_APP_VERSION',FTP3.VERSION);
  SpreadsheetApp.getActive().toast('FlipTracker Pro upgraded to schema 8.0.','FlipTracker Pro',6);
}
