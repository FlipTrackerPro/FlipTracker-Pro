const FTP3 = {
  ...
  MILEAGE_HEADERS: [
    ...
    'Total Kilometres','Business Kilometres','CRA Rate','Claim Amount','Notes','Created At'
  ],
  PACKAGING_HEADERS: [
...
function buildMileageSprint3_() {
  ...
  ['Start Odometer','End Odometer','Google Maps Distance','Total Kilometres','Business Kilometres']
    .forEach(h=>s.getRange(2,map[h],FTP3.ROWS,1).setNumberFormat('0'));
  ['CRA Rate','Claim Amount']
    .forEach(h=>s.getRange(2,map[h],FTP3.ROWS,1).setNumberFormat('$#,##0.00'));
  s.getRange(2,map['Created At'],FTP3.ROWS,1).setNumberFormat('yyyy-mm-dd hh:mm');

  const yesNo=SpreadsheetApp.newDataValidation()
    .requireValueInList(['No','Yes'],true).setAllowInvalid(false).build();
  ...
  ['Start Location','End Location','Business Purpose','Vehicle','Notes']
    .forEach(h=>s.setColumnWidth(map[h],180));
}

function migrateMileageSchema62_(s) {
  if(s.getLastRow()<1)return;
  const oldHeaders=s.getRange(1,1,1,s.getLastColumn()).getDisplayValues()[0];
  if(!oldHeaders.some(Boolean))return;
  if(oldHeaders.join('|')===FTP3.MILEAGE_HEADERS.join('|'))return;

  const aliases={
    'Start Location':['Start'],
    'End Location':['Destination','End'],
    'Start Odometer':['Odometer Start','Mileage Start'],
    'End Odometer':['Odometer End','Mileage End']
  };
  const oldMap={};
  oldHeaders.forEach((h,i)=>{if(h)oldMap[String(h).trim()]=i;});
  const rowCount=Math.max(0,s.getLastRow()-1);
  const oldRows=rowCount?s.getRange(2,1,rowCount,s.getLastColumn()).getValues():[];
  const newRows=oldRows.map(row=>FTP3.MILEAGE_HEADERS.map(header=>{
    const candidates=[header].concat(aliases[header]||[]);
    for(const name of candidates){
      if(Object.prototype.hasOwnProperty.call(oldMap,name))return row[oldMap[name]];
    }
  ...
  s.clear();
  ensureSize3_(s,Math.max(FTP3.ROWS+1,newRows.length+1),FTP3.MILEAGE_HEADERS.length);
  s.getRange(1,1,1,FTP3.MILEAGE_HEADERS.length).setValues([FTP3.MILEAGE_HEADERS]);
...
function showRecordMileageForm() {
  ...
  input,select,textarea{width:100%;box-sizing:border-box;padding:8px;margin-top:3px;border:1px solid #B7C9D6;border-radius:4px}
  .grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}button{margin-top:16px;padding:10px 16px;border:0;border-radius:4px;background:#1F4E78;color:#fff;font-weight:700}
  .hint{font-size:12px;color:#667085;margin-top:3px}</style></head><body><form id="f"><div class="grid">
  <div><label>Date</label><input type="date" name="date" required></div>
  <div><label>CRA rate per km</label><input type="number" step="0.01" min="0" name="craRate"></div>
  <div><label>Start location</label><input name="startLocation" placeholder="Home"></div>
  <div><label>End location</label><input name="endLocation" placeholder="Post office"></div>
  <div><label>Round trip</label><select name="roundTrip"><option>No</option><option>Yes</option></select></div>
  <div><label>Vehicle</label><input name="vehicle" placeholder="Honda CR-V"></div>
  <div><label>Start odometer</label><input type="number" step="1" min="0" name="odoStart"></div>
  <div><label>End odometer</label><input type="number" step="1" min="0" name="odoEnd"></div>
  <div><label>Google Maps distance (km)</label><input type="number" step="1" min="0" name="mapsDistance"><div class="hint">Optional. Overrides odometer distance. Round Trip doubles it.</div></div>
  <div><label>Business kilometres</label><input type="number" step="1" min="0" name="businessKm"><div class="hint">Leave blank to use the full trip distance.</div></div>
  </div>
  <label>Business purpose</label><input name="purpose" required placeholder="Sourcing inventory or shipping orders">
  <label>Notes</label><textarea name="notes" rows="3"></textarea>
...
function saveMileage3_(form) {
  ...
  const odoStart=Math.round(num3_(form.odoStart));
  const odoEnd=Math.round(num3_(form.odoEnd));
  const odometerDistance=Math.max(0,odoEnd-odoStart);
  const mapsEntered=form.mapsDistance!==''&&form.mapsDistance!=null;
  const mapsDistance=mapsEntered?Math.max(0,Math.round(num3_(form.mapsDistance))):'';
  const roundTrip=String(form.roundTrip||'No')==='Yes'?'Yes':'No';
  const total=mapsEntered?mapsDistance*(roundTrip==='Yes'?2:1):odometerDistance;
  const requestedBusiness=form.businessKm===''||form.businessKm==null
    ?total:Math.round(num3_(form.businessKm));
  ...
  const claim=roundMoney3_(business*rate);

  const record={
    'Trip ID':nextId3_(FTP3.SHEETS.MILEAGE,1,'TRP'),
    'Date':date3_(form.date),
    'Start Location':form.startLocation||'',
    'End Location':form.endLocation||'',
    'Business Purpose':form.purpose||'',
    'Round Trip':roundTrip,
    'Vehicle':form.vehicle||'',
    'Start Odometer':odoStart,
    'End Odometer':odoEnd,
    'Google Maps Distance':mapsDistance,
    'Total Kilometres':total,
    'Business Kilometres':business,
    'CRA Rate':rate,
    'Claim Amount':claim,
    'Notes':form.notes||'',
    'Created At':new Date()
  ...
  const s=sheet3_(FTP3.SHEETS.MILEAGE);
  const row=Math.max(s.getLastRow()+1,2);
  s.getRange(row,1,1,FTP3.MILEAGE_HEADERS.length)
    .setValues([FTP3.MILEAGE_HEADERS.map(h=>record[h])]);
  refreshDashboardSprint3();
}
...
function buildDashboardSprint3_() {
  ...
  const mClaim=sheetColumnLetter3_(FTP3.SHEETS.MILEAGE,'Claim Amount'),mDate=sheetColumnLetter3_(FTP3.SHEETS.MILEAGE,'Date'),mPurpose=sheetColumnLetter3_(FTP3.SHEETS.MILEAGE,'Business Purpose'),mKm=sheetColumnLetter3_(FTP3.SHEETS.MILEAGE,'Business Kilometres');
  const cards=[
    ...
    ['E8:F10','Mileage Claims',`SUM(Mileage!${mClaim}2:${mClaim})`,'$#,##0.00'],
  ...
  s.getRange('E20:H20').merge().setFormula(`=SUM(Sales!${sProfit}2:${sProfit})-SUM(Expenses!J2:J)-IF(IFERROR(VLOOKUP("Include Mileage Estimate in Expenses",Settings!A:B,2,FALSE),"No")="Yes",SUM(Mileage!${mClaim}2:${mClaim}),0)`);
  ...
  s.getRange('A29:D29').merge().setValue('Business Kilometres by Month');header3_(s.getRange('A29:D29'));
  s.getRange('A30').setFormula(`=QUERY({TEXT(Mileage!${mDate}2:${mDate},"yyyy-mm"),Mileage!${mKm}2:${mKm}},"select Col1,sum(Col2) where Col1 is not null group by Col1 label Col1 'Month',sum(Col2) 'Business KM'",0)`);

  s.getRange('E29:H29').merge().setValue('Business Kilometres by Purpose');header3_(s.getRange('E29:H29'));
  s.getRange('E30').setFormula(`=QUERY({Mileage!${mPurpose}2:${mPurpose},Mileage!${mKm}2:${mKm}},"select Col1,sum(Col2) where Col1 is not null group by Col1 order by sum(Col2) desc label Col1 'Purpose',sum(Col2) 'Business KM'",0)`);
...
function installFlipTrackerPro() {
  ...
  repairWholeNumberValues3_();
  applyFlipTrackerNumberFormats3_();
  p.setProperty('FTP_SCHEMA_VERSION','6.2'); p.setProperty('FTP_APP_VERSION',FTP3.VERSION);
  goToDashboardSprint3();
  SpreadsheetApp.getActive().toast('FlipTracker Pro v0.6.2 is ready.','FlipTracker Pro',6);
}
...
function upgradeFlipTrackerPro() {
  ...
  applyFlipTrackerNumberFormats3_();
  p.setProperty('FTP_SCHEMA_VERSION','6.2'); p.setProperty('FTP_APP_VERSION',FTP3.VERSION);
  SpreadsheetApp.getActive().toast('FlipTracker Pro upgraded to schema 6.2.1.','FlipTracker Pro',6);
}
